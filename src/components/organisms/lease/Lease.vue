  ...
  <div class="flex w-full flex-col border-[1px] border-border-color bg-neutral-bg-2 p-6 px-4 lg:rounded-xl lg:px-6">
    ...
    <div class="flex flex-col gap-6 lg:h-[150px] lg:flex-row">
      ...
      >
        ...
        >
          <div class="text-12 font-medium text-typography-secondary">PnL</div>
          <div
            :class="[
              {
                '!bg-error-emphasized !text-typography-static-light': pnl.status === LeasePnlStatus.NEGATIVE,
                '!bg-success-emphasized !text-typography-static-light': pnl.status === LeasePnlStatus.POSITIVE,
        ...
        >
          <template v-if="progressBar?.value.length === 1 && status !== LeaseStatus.OPENING">
            <div class="text-12 font-medium text-typography-secondary">{{ progressBar.title }}</div>
            <div class="flex h-full items-center justify-between gap-1">
              <div
                :class="[{ '!bg-error-emphasized': +progressBar.value > 80 }]"
                class="h-[10px] w-[24px] rounded bg-progress-color lg:w-[35px]"
              ></div>
              <div
                :class="[{ '!bg-warning-emphasized': +progressBar.value > 65 && +progressBar.value <= 80 }]"
                class="h-[10px] w-[24px] rounded bg-progress-color lg:w-[35px]"
              ></div>
              <div
                :class="[{ '!bg-success-emphasized': +progressBar.value <= 65 }]"
                class="h-[10px] w-[24px] rounded bg-progress-color lg:w-[35px]"
              ></div>
            </div>
          </template>
            ...
            <div class="flex justify-between gap-1">
              <div
                ...
                :class="[
                  ...
                  {
                    'pulse !bg-primary-default': val === LeaseOpeningBarStatuses.CURRENT,
                    '!bg-primary-default': val === LeaseOpeningBarStatuses.READY
                  }
                ]"
              ></div>
            </div>
        ...
        <div class="flex flex-col gap-1">
          <div class="text-12 font-medium text-typography-secondary">{{ progressDate.title }}</div>
          <div
    ...
    <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between lg:gap-0">
      <div class="flex items-center justify-between lg:justify-normal lg:gap-8">
        <template v-if="debt">
          <div>
            <div class="flex items-center text-12 font-medium text-typography-secondary">
              {{ debt.title }}
              <Tooltip
